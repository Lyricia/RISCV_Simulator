	...
	Elf64_Byte st_other, Elf64_Half st_shndx, Elf64_Addr st_value)
{
	...
	Elf64_Sym es;
	es.st_name = st_name;
	es.st_info = ELF64_ST_INFO(st_bind, st_type);
	es.st_other = st_other;
	es.st_shndx = st_shndx;
	es.st_value = st_value;
	symbols.push_back(es);
	...
	Elf64_Xword r_type, Elf64_Sxword r_addend)
{
	...
	Elf64_Rela er;
	er.r_offset = r_offset;
	er.r_info = ELF64_R_INFO(r_sym, r_type);
	er.r_addend = r_addend;

	relocations.push_back(er);
	return i;
}
